...
# 10/7/15
# CIS 125 82A
# In collaboration with Trevor Waite, Josiah Hardacre, and Nathan Pugh
#!/usr/bin/env python
# encoding: utf-8
"""
GameOfWar.py
...
def main():
	...
	while len(PlayerAHand) > 0 and len(PlayerBHand) > 0:
		gameCounter += 1
	# Stops game if it plays more than 1000 rounds
		if gameCounter > 999:
...
def playRound(PlayerA, PlayerB):
	...
	if AsRank > BsRank:
		...
		PlayerA.append(AsCard)
		PlayerB.append(BsCard)
...
def WAR(PlayerA, PlayerB):
	if len(PlayerA) > 5 and len(PlayerB) > 5:
		...
		BsHand = []
		for y in range(5):
			AsHand.append(PlayerA.pop())
			BsHand.append(PlayerB.pop())

		if getRank(AsHand[4]) > getRank(BsHand[4]):
			PlayerA = AsHand + BsHand + PlayerA
		elif getRank(AsHand[4]) > getRank(BsHand[4]):
			PlayerB =  AsHand + BsHand + PlayerB
		else:
